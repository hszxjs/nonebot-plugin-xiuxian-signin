...
function PlayerListCard({
  ...
  return (
    <Card title={title} extra={<Typography.Text type="secondary">{description}</Typography.Text>}>
      <List
        dataSource={players}
        renderItem={(player) => (
          <List.Item actions={[<Tag key="power">{formatCompactNumber(player.battle_power)}</Tag>]}>
            <List.Item.Meta
              title={
                <Space wrap>
...
export function DashboardWorkspace({
  ...
  const signinPercent = Math.round(dashboard.health_flags.today_signin_ratio * 100)
  const inactivePercent = Math.round(dashboard.health_flags.inactive_ratio * 100)
  ...
  return (
    <div className="workspace-stack">
      ...
      <div className="two-column">
        <Card className="chart-card" title="境界分布" extra="按当前玩家存档生成的境界快照">
          <Bar
          ...
          <Space orientation="vertical" size="large" className="full-width">
            ...
            <div>
              <Space className="full-width" orientation="vertical">
                ...
                <Progress percent={inactivePercent} status={inactivePercent > 25 ? "exception" : "normal"} />
              </Space>
            </div>
            <ConfirmAction
              triggerLabel="创建玩家备份"
      ...
      <Card title="资源汇总" extra={<Tag>{dashboard.capabilities.snapshot_dashboard ? "快照" : "实时"}</Tag>}>
        <div className="metric-grid">
